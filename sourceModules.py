    ...
    for key in resp:
        for items in resp[key]:
            ...
            if items['name'].startswith(sqlinstance_name):
                sqlinstance['name'] = items['name']
                sqlinstance['email'] = items['serviceAccountEmailAddress']
                if items['ipAddresses'][0]['type']=='PRIMARY':
                    sqlinstance['ip'] = [items['ipAddresses'][0]['ipAddress']]
                sqlinstance['state'] = items['state']
            if sqlinstance:
                sqlinstances.append(sqlinstance)
    return sqlinstances
# [END list_sql_instances]
# list_sql_instances(cloudsql,"ti-is-devenv-01","loveit")
...
def import_sqlinstance(cloudsql, project, sqlinstance_name, uri, database_name, filetype):
    config = {
        'importContext': {
            'uri': uri,
    ...
    return cloudsql.instances().import_(
    	...
    	body=config).execute()
# [END import_instance]
# import_sqlinstance(cloudsql,"ti-is-devenv-01","us-west1-a",instances['name'],uri,'EvoDb_Testing','BAK')
...
#    add_bucket_iam_member(bucket,"roles/storage.admin","serviceAccount:" + instances['email'])
#
#    if wait:
#        input()
#
...
#
#    operation = import_sqlinstance(cloudsql, project, instances['name'],uri,'EvoDb_Testing','BAK')
#    wait_for_operation(cloudsql, project, operation['name'])
#
#
#    print("""
